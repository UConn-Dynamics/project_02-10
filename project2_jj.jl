...
begin
    ...
    bar_length_m = 0.10     # 10 cm in m
    bar_rate_rad_s = 2.0
    rotation_period_s = 2π / bar_rate_rad_s
end
...
# ╔═╡ 9d6b1076-fbd9-46c8-96ba-9ff20a300d85
begin
    unit_track1 = (1 / √2) .* [1.0,  1.0]   # +45 direction
    unit_track2 = (1 / √2) .* [1.0, -1.0]   # -45 direction

    # Constraint 
    constraint_matrix_A = (1 / √2) .* [ 1.0  -1.0
                                        1.0   1.0 ]
end

# ╔═╡ 1f1d1797-ddbd-42f8-ba16-96d5f6377a67
begin
    theta3(time_s) = bar_rate_rad_s * time_s

    function slider_kinematics(time_s)
        θ = theta3(time_s)

        right_hand_b = bar_length_m .* [cos(θ), sin(θ)]
        slider_s = constraint_matrix_A \ right_hand_b

        right_hand_bdot = bar_length_m * bar_rate_rad_s .* [-sin(θ), cos(θ)]
        slider_sdot = constraint_matrix_A \ right_hand_bdot

        right_hand_bddot = bar_length_m * bar_rate_rad_s^2 .* [-cos(θ), -sin(θ)]
        slider_sddot = constraint_matrix_A \ right_hand_bddot

        return slider_s, slider_sdot, slider_sddot
    end
end

# ╔═╡ adf25ac5-8fdd-4c78-a7fc-619749cae730
begin
    function piston_positions(time_s)
        slider_s, _, _ = slider_kinematics(time_s)
        slider1_s, slider2_s = slider_s[1], slider_s[2]

        position_piston1 = slider1_s .* unit_track1
        position_piston2 = slider2_s .* unit_track2
        return position_piston1, position_piston2
    end
end

# ╔═╡ 94695502-bfbd-4ad1-b3a3-a19b0a54c2c4
begin
    track_extent_m = 0.12
    ...
    anim = @animate for time_s in time_grid_s
        p1, p2 = piston_positions(time_s)

        plot(track_line, track_line; label=false, aspect_ratio=1,
            xlim=(-track_extent_m, track_extent_m), ylim=(-track_extent_m, track_extent_m),
            xlabel="x (m)", ylabel="y (m)")
        plot!(track_line, -track_line; label=false)

        plot!([p1[1], p2[1]], [p1[2], p2[2]]; lw=4, label=false)
        scatter!([p1[1], p2[1]], [p1[2], p2[2]]; ms=6, label=false)
    end

    gif(anim, "dual_slider_method3.gif"; fps=30)
end
